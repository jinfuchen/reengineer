...
To conduct our research, we extract the domain commit-level change measures and the file-level performance-relevant change measures from the CVS repositories of the projects. The overview of the change measures is shown in Table~\ref{tab:measures}.
 ...
 \textbf{Commit-level change measures.} Inspired by previous studies from \textit{Kamei et al.} and \textit{Mockus et al.}, we inherit their change measures and extend these measrues to performance prediction area. We consider 12 change measures grouped into five dimensions.
\begin{enumerate}[(i)]
	\item \noindent
